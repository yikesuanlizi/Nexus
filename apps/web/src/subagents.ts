...
import type { AgentStageRow, SubagentStatusRow, ThreadChildInfo } from './types.js';
  ...
  return [
    {
      ...
      parentThreadId: '',
      title: options.activeThreadTitle || (options.locale === 'zh' ? '主 Agent' : 'Main Agent'),
      role: options.locale === 'zh' ? '主 Agent' : 'Main Agent',
      depth: 0,
      status: mainStatus,
      statusLabel: labels[mainStatus],
      tone: statusTone[mainStatus],
      latestAction: options.busy
        ? (options.locale === 'zh' ? '正在处理当前任务' : 'Working on the current task')
        : (options.locale === 'zh' ? '等待输入' : 'Waiting for input'),
