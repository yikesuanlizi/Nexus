...
export function mergeIncomingItems(current: ThreadItem[], incoming: ThreadItem[]): ThreadItem[] {
  ...
  const hasRealUserMessage = normalized.some((item) => item.type === 'user_message' && !item.id.startsWith('pending_user_'));
  const base = hasRealUserMessage
    ? current.filter((item) => !(item.type === 'user_message' && item.id.startsWith('pending_user_')))
    : current;
  return mergeThreadItems(base, normalized) as ThreadItem[];
}
