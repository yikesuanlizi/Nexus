...
export async function controlThreadWorkflow(
...
): Promise<{ ok?: boolean; thread?: ThreadMeta; workflow?: WorkflowSnapshot; components?: WorkflowComponentDefinition[]; blueprint?: WorkflowBlueprintCompileResult | null; events?: unknown[]; run?: WorkflowRun; error?: string }> {
  ...
  const data = (await response.json()) as { ok?: boolean; thread?: ThreadMeta; workflow?: WorkflowSnapshot; components?: WorkflowComponentDefinition[]; blueprint?: WorkflowBlueprintCompileResult | null; events?: unknown[]; run?: WorkflowRun; error?: string };
