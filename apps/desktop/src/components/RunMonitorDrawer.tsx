        ...
        <div className="runMonitorRunList" aria-label={zh ? '运行列表' : 'Run list'}>
          {runs.length === 0 ? (
            ...
            >
              ...
              <small>{run.tenantId} · {run.status} · {run.activeStep ?? run.kind}</small>
            </button>
          ))}
        </div>
            ...
            <>
              <div className="runMonitorSelected">
                <div>
                  ...
                  <span>{selectedRun.tenantId} · {selectedRun.threadId}</span>
                </div>
                ...
                {visibleEvents.length === 0 ? (
                  ...
                  <article className={`runMonitorEvent ${event.level}`} key={event.eventId}>
                    <div>
                      ...
                      <span>{event.category}{event.toolName ? ` · ${event.toolName}` : ''}</span>
                    </div>
                    <p>{event.message}</p>
